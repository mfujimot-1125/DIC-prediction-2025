with
    ...
    urinary_tract as (
        select icu_stay_id, 1 as urinary_tract_flag
        from {{ ref("medicu", "one_icu_icu_diagnoses") }}
        where
            (
                icd10 like 'N308%'
...
select
    ...
    ifnull(nervous_system_flag, 0) as infected_nervous_system,
    ifnull(cardiovascular_flag, 0) as infected_cardiovascular,
    ifnull(respiratory_flag, 0) as infected_respiratory,
    ifnull(abdomen_flag, 0) as infected_abdomen,
    ifnull(urinary_tract_flag, 0) as infected_urinary_tract,
    ifnull(soft_tissue_flag, 0) as infected_soft_tissue,
    ifnull(other_flag, 0) as infected_other
from icu_stays
left join charlson_comorbidity_index using (icu_stay_id)
